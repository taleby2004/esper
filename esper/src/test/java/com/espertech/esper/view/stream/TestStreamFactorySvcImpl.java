...
public class TestStreamFactorySvcImpl extends TestCase
{
    ...
    public void testInvalidJoin()
    {
        ...
        streams[0] = streamFactoryService.createStream(filterSpecs[0], supportFilterService, null, true, false, false).getFirst();

        try
        {
            // try to reuse the same filter spec object, should fail
            streamFactoryService.createStream(filterSpecs[0], supportFilterService, null, true, false, false);
            fail();
        }
    ...
    public void testCreateJoin()
    {
        streams = new EventStream[3];
        streams[0] = streamFactoryService.createStream(filterSpecs[0], supportFilterService, null, true, false, false).getFirst();
        streams[1] = streamFactoryService.createStream(filterSpecs[1], supportFilterService, null, true, false, false).getFirst();
        streams[2] = streamFactoryService.createStream(filterSpecs[2], supportFilterService, null, true, false, false).getFirst();
    ...
    public void testCreateNoJoin()
    {
        ...
        streams[0] = streamFactoryService.createStream(filterSpecs[0], supportFilterService, stmtHande, false, false, false).getFirst();
        streams[1] = streamFactoryService.createStream(filterSpecs[0], supportFilterService, stmtHande, false, false, false).getFirst();
        streams[2] = streamFactoryService.createStream(filterSpecs[1], supportFilterService, stmtHande, false, false, false).getFirst();
        streams[3] = streamFactoryService.createStream(filterSpecs[2], supportFilterService, stmtHande, false, false, false).getFirst();
