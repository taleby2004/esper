...
public class TestAccessAggMinMaxBySorted extends TestCase {
    ...
    public void testInvalid() {
        tryInvalid("select maxBy(p00||p10) from S0.std:lastevent(), S1.std:lastevent()",
                "Error starting statement: The 'maxby' aggregation function requires that any parameter expressions evaluate properties of the same stream [select maxBy(p00||p10) from S0.std:lastevent(), S1.std:lastevent()]");

        tryInvalid("select sorted(p00) from S0",
                "Error starting statement: The 'sorted' aggregation function requires that a data window is declared for the stream [select sorted(p00) from S0]");
    }
