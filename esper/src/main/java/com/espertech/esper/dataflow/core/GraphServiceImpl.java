...
public class GraphServiceImpl implements GraphService {
    private static final Log log = LogFactory.getLog(GraphServiceImpl.class);

    private static final String EVENT_WRAPPED_TYPE = "eventbean";

    private final Map<String, GraphServiceEntry> graphs = new HashMap<String, GraphServiceEntry>();

    private final EPServiceProvider epService;

    public GraphServiceImpl(EPServiceProvider epService) {
    ...
    public synchronized void addStartGraph(CreateDataFlowDesc desc, StatementContext statementContext, EPServicesContext servicesContext, AgentInstanceContext agentInstanceContext, boolean newStatement) throws ExprValidationException {
        ...
        GraphServiceEntry existing = graphs.get(desc.getGraphName());
        ...
        GraphStmtDesc stmtDesc = new GraphStmtDesc(desc, statementContext, servicesContext, agentInstanceContext, operatorAnnotations);
        graphs.put(desc.getGraphName(), new GraphServiceEntry(stmtDesc, EPStatementState.STARTED));
    }

    public void stopGraph(String graphName) {
        GraphServiceEntry existing = graphs.get(graphName);
    ...
    public EPDataFlowInstance instantiate(String dataFlowName, EPDataFlowInstantiationOptions options) {
        final GraphServiceEntry serviceDesc = graphs.get(dataFlowName);
        if (serviceDesc == null) {
        ...
        GraphStmtDesc stmtDesc = serviceDesc.getGraphDesc();
        try {
                                                   ...
                                                   Map<GraphOperatorSpec, Annotation[]> operatorAnnotations) throws ExprValidationException
    {
        ...
        GraphStartDesc startDesc = RealizationFactoryInterface.realize(dataFlowName, operators, operatorMetadata, operatorBuildOrder, operatorChannelBindings, dataFlowSignalManager, options, servicesContext, statementContext);
    ...
    private Object instantiateOperator(String dataFlowName, int operatorNum, OperatorMetadataDescriptor desc, GraphOperatorSpec graphOperator, EPDataFlowInstantiationOptions options, EngineImportService engineImportService)
        ...
        injectObjectProperties(dataFlowName, graphOperator.getOperatorName(), operatorNum, configs, operatorObject, options.getParameterProvider(), engineImportService);
    ...
    private void injectObjectProperties(String dataFlowName, String operatorName, int operatorNum, Map<String, Object> configs, Object instance, EPDataFlowOperatorParameterProvider optionalParameterProvider, EngineImportService engineImportService)
        throws ExprValidationException {
        ...
        PopulateUtil.populateObject(operatorName, operatorNum, dataFlowName, configs, propertyInstance, engineImportService, optionalParameterProvider);
