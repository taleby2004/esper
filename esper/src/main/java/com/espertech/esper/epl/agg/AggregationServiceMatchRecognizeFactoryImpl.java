        ...
        for (int stream = 0; stream < factoryEachStream.length; stream++) {
            ...
            if (thatStream != null) {
                ...
                for (int aggId = 0; aggId < thatStream.length; aggId++) {
                    aggregatorsEachStream[stream][aggId] = factoryEachStream[stream][aggId].make(agentInstanceContext.getStatementContext().getMethodResolutionService(), agentInstanceContext.getAgentInstanceIds(), 0, aggId);
                    count++;
                }
            }
        }
