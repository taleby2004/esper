...
public class EPStatementStartMethodSelect extends EPStatementStartMethodBase
{
    ...
    public EPStatementStartResult startInternal(boolean isNewStatement, boolean isRecoveringStatement, boolean isRecoveringResilient) throws ExprValidationException, ViewProcessingException {
        ...
        for (int i = 0; i < statementSpec.getStreamSpecs().size(); i++)
        {
            ...
            else if (streamSpec instanceof PatternStreamSpecCompiled)
            {
                ...
                final PatternContext patternContext = statementContext.getPatternContextFactory().createContext(statementContext, i, rootFactoryNode, !patternStreamSpec.getArrayEventTypes().isEmpty());
        ...
        if (numStreams > 1) {
            joinSetComposerPrototype = JoinSetComposerPrototypeFactory.makeComposerPrototype(
                    ...
                    joinAnalysisResult, queryPlanLogging, statementContext.getAnnotations(), historicalViewableDesc, agentInstanceContext);
        }
     ...
     * @throws com.espertech.esper.epl.expression.ExprValidationException if constraints violated
     */
