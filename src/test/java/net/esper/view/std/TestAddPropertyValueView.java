...
public class TestAddPropertyValueView extends TestCase
{
    ...
    public void setUp()
    {
        ...
        parentEventType = EventTypeFactory.getInstance().createMapType(schema);
    ...
    public void testViewUpdate()
    {
        ...
        EventBean eventBeanOne = EventBeanFactory.createMapFromValues(eventData, parentEventType);
        eventData.put("STDDEV", 0);
        EventBean eventBeanTwo = EventBeanFactory.createMapFromValues(eventData, parentEventType);
        eventData.put("STDDEV", 99999);
        EventBean eventBeanThree = EventBeanFactory.createMapFromValues(eventData, parentEventType);
    ...
    public void TestAddProperty()
    {
        Map<String, Object> eventData = new HashMap<String, Object>();
        eventData.put("STDDEV", 100);
        EventBean eventBean = EventBeanFactory.createMapFromValues(eventData, parentEventType);

        EventType newEventType = EventTypeHelper.createAddToEventType(parentEventType, new String[] {"test"}, new Class[] {Integer.class});
        EventBean newBean = AddPropertyValueView.addProperty(eventBean, new String[] {"test"}, new Object[] {2}, newEventType);
